...
**Status: `APPROVED — PLAN OF RECORD; S7-0 COMPLETE; NO IMPLEMENTATION CHECKPOINT APPROVED`**
...
section of `docs/harvest/TODO.md` — **is complete through this commit, and it is the only approved
Stage 7 checkpoint.**

**`S7-1` … `S7-C` remain unapproved. Stage 7 implementation has not begun**, and nothing in this
document begins it: not one line of migration code exists, and none may be written until the
checkpoint that owns it is approved by name.
...
**None of these is approved.** Each requires separate approval by name, and each is limited to the
exact path set below. Every checkpoint includes updates to this plan and to `docs/harvest/TODO.md`.

### S7-1 · Entity assessment

...
S1="$ROOT/agents/stage1"; STATE="${STATE_DIR:-$ROOT/state}"
mkdir -p "$STATE"
...
run_1f() {
  ...
  if jq -s '{ledger: ((.[0].ledger // []) + (.[1].ledger_updates // [])) | unique_by(.url)}' \
    "$STATE/visited_url_ledger.json" "$STATE/news_hits.json" > "$STATE/ledger.tmp"; then
    mv "$STATE/ledger.tmp" "$STATE/visited_url_ledger.json"
  else
    rm -f "$STATE/ledger.tmp"
    echo "ERROR: [1F] ledger merge failed; $STATE/visited_url_ledger.json left unchanged." >&2
    exit 1
  fi
  ...
  if [ "${n_hits:-0}" -gt 0 ]; then
    ...
    if jq -s '(.[1].ledger_patch // []) as $p
       ...
       "$STATE/visited_url_ledger.json" "$STATE/news_case_db_with_patch.json" > "$STATE/ledger.tmp"; then
      mv "$STATE/ledger.tmp" "$STATE/visited_url_ledger.json"
    else
      rm -f "$STATE/ledger.tmp"
      echo "ERROR: [1C-news] ledger patch merge failed; $STATE/visited_url_ledger.json left unchanged." >&2
      exit 1
    fi
    ...
    "$CLAUDE_BIN" -p "Follow your system instructions. Hits: $STATE/news_hits.json. Visited-URL ledger: $STATE/visited_url_ledger.json (use its entity_extracted/entity_ids fields, separate from 1C's extracted/case_ids on the same rows). Output ONLY the entity batch JSON (entities, ledger_patch). No prose, no fences." \
    ...
    if jq -s '(.[1].ledger_patch // []) as $p
           | {ledger: [ .[0].ledger[] | . as $e | (($p[] | select(.url==$e.url)) // {}) as $u | $e + $u ]}' \
       "$STATE/visited_url_ledger.json" "$STATE/news_entity_batch.json" > "$STATE/ledger.tmp"; then
      mv "$STATE/ledger.tmp" "$STATE/visited_url_ledger.json"
    else
      rm -f "$STATE/ledger.tmp"
      echo "ERROR: [1G-news] ledger patch merge failed; $STATE/visited_url_ledger.json left unchanged." >&2
      exit 1
    fi
...
run_1e() {
  ...
  if "$CLAUDE_BIN" -p "Follow your system instructions. Today is $(date -u +%Y-%m-%d). Read state/search_hits.json (and its per-topic shards), state/community_strategy_db.json's discovery_candidates[] if present, and state/category_registry.json (augment, do not clobber). Current SEED_TOPICS: $SEED_TOPICS. Propose new candidate categories with >=3 evidence points; never set status to active yourself. Output ONLY the full updated category_registry.json. No prose, no fences." \
    --append-system-prompt "$(cat "$S1/1E_category_discovery.md")" --allowedTools "Read,WebSearch" "${FLAGS[@]}" \
    2> "$STATE/1E.err" | jq -r '.result' | clean > "$STATE/category_registry.json.tmp"; then
    mv "$STATE/category_registry.json.tmp" "$STATE/category_registry.json"
  else
    rm -f "$STATE/category_registry.json.tmp"
    echo "ERROR: [1E] category discovery step failed; $STATE/category_registry.json left unchanged." >&2
    exit 1
  fi

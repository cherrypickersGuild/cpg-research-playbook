...
for f in "${FILES[@]:-}"; do
  ...
  case "$f" in
    ...
    scripts/harvest_entities.sh)                  add_test tests/test_harvest_bounded.sh; add_test tests/test_harvest_targets.sh; add_test tests/test_entity_github_stars.sh; add_test tests/test_clean_json.sh ;;

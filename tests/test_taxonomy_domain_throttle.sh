#!/usr/bin/env bash
# test_taxonomy_domain_throttle.sh — the per-domain cap must hold ACROSS
# PROCESSES. Launches real subprocesses against a local recording HTTP server and
# measures observed concurrency and inter-arrival gaps. Takes ~15s.
...
python -m unittest discover -s tests/harvest -p 'test_domain_throttle.py' -v
EC=$?

#!/usr/bin/env bash
# test_taxonomy_identity.sh — canonicalization, identity stability, fragment and
# alias policy. Thin wrapper so scripts/validate_task.sh (which only executes
...
python -m unittest discover -s tests/harvest -p 'test_identity.py' -v
EC=$?

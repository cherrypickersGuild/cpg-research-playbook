...
The playbook discovers credible AI-transformation cases and catalogs agent/mcp/prompt/skill entities via
a 7-agent Stage 1 sub-pipeline, then validates, selects, and builds lecture-ready slide decks (Stages 2–4).

```
seed topics → 1A Strategy Builder → 1B Crawl Executor ─┬→ 1C Case Extractor    → state/ax_case_db.json
                     ↑                    ↑            └→ 1G Entity Extractor  → state/entity_registry.json
                     │                    │
                     │            1F News Monitor (fast lane, tier:"news" sources) → feeds 1C + 1G directly
                     │
                     └──────── 1D Refresh Scheduler (closed cycle, evergreen sources) ────────┘

1E Category Discovery (slow lane) → state/category_registry.json → human review → new SEED_TOPICS
```

Both `state/ax_case_db.json` and `state/entity_registry.json` are cumulative masters, deduped by key
(`case_key` / `entity_key`) and merged across every run — not per-run snapshots. See
`docs/SEEDING_STRATEGY.md` for the full strategy, schedule, and workflow behind 1D-1G.
...
- `agents/stage1/1E_category_discovery.md` / `1F_news_monitor.md` / `1G_entity_extractor.md` — slow-cadence new-topic discovery, fast-cadence news crawling, and 1C's sibling extractor for agent/mcp/prompt/skill entities; see `docs/SEEDING_STRATEGY.md` for the full plan.
- `scripts/run_stage1.sh` — runs the discovery sub-pipeline.

### Seeding strategy
- `docs/SEEDING_STRATEGY.md` — the strategy, schedule, and workflow for growing seed topics/categories, sources, and news coverage over time, and for merging newly discovered cases into a persistent `state/ax_case_db.json` instead of leaving them as disconnected per-run snapshots.
- `scripts/discover.sh` — runs 1F News Monitor and/or 1E Category Discovery (`--news-only` / `--category-only` / default both); mirrors `scripts/refresh.sh`.
- `scripts/merge_case_db.sh` — folds a batch of extracted cases into `state/ax_case_db.json`, deduped by `case_key`, raising `corroboration_count` on repeat matches and logging (never silently resolving) conflicting facts.
- `scripts/merge_entity_registry.sh` — same pattern for `state/entity_registry.json`: structured agent/mcp/prompt/skill records extracted by 1G, deduped by `entity_key`, upgrading `description_source` on corroboration rather than downgrading it.
- `scripts/calibrate_seeding.sh` — seeding-health metrics read from `state/` directly: yield by topic, corroboration distribution, category discovery funnel, news-vs-evergreen yield, staleness.
- `schedule/crontab.txt` / `schedule/register_windows_tasks.ps1` — install the six-tier schedule from `docs/SEEDING_STRATEGY.md` §5 on Linux/macOS/WSL or Windows Task Scheduler.
...
bash scripts/run_stage1.sh              # one discovery pass: 1A -> 1B -> 1C + 1G, merged into both master DBs
bash scripts/refresh.sh                 # 1D closed cycle: re-runs only stale evergreen strategies
bash scripts/discover.sh                # 1F news (-> 1C + 1G) and 1E category discovery; --news-only / --category-only
bash scripts/calibrate_seeding.sh       # seeding health: yield, corroboration, category funnel, staleness
```
...
- **2026-07-06** — Reorganized the repo root: all 8 pipeline shell scripts moved into `scripts/`, `SETUP.md` and `SEEDING_STRATEGY.md` moved into `docs/` (README.md/CLAUDE.md/pipeline.config.sh stay at root). Updated `ROOT` resolution and every inter-script call site accordingly, plus all path references in this file, `CLAUDE.md`, `docs/SETUP.md`, and `schedule/`. Re-ran `discover.sh` end-to-end post-move to confirm nothing broke.
